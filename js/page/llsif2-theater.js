...
function DrawStoryList(conditions){
	...
	document.getElementById("StoryContainer").innerHTML = filteredData.map( story => {
		...
		const tagContent = story.tags.map( tag => tag in TagData ? LLS.createStyledTag(TagData[tag], tag) : '').join('');
		...
		<article class="story">
			...
			<div class="story-titleContainer">
				...
				<div class="story-memo">${('memo' in story ? LLS.markup(story.memo) : '')}</div>
			</div>
...
function MakeModal(story_id){
	...
	result.text = LLS.markup(result.text);
...
function initialize() {
	...
	const characterList = LLSIdol.filterCharacterList("has:color_llsif2");
	...
	document.querySelector('style').textContent += "\n<!--" + Object.keys(TagData).map( character => {
		return `
		.button-${character} {
			background-color: ${LLS.getColorFromObject(TagData[character], 2)};
			border-color: ${LLS.getColorFromObject(TagData[character], 0)}
		}`;
	}).join('') + "\n-->";
