...
function WriteMessage(page) {
	...
	const Output = CurrentSet.opponents.map((opponent) => {
		...
		const targetCharacter = LLSIdolManager.getCharacterData("llsif", opponent.id)
		?? LLSIdolManager.getCharacterData("llsif_collab", opponent.id)
		?? LLSIdolManager.getCharacterData("a-rise", opponent.id)
		?? LLSIdolManager.getCharacterData("saintsnow", opponent.id) ;

		//ステージ数
		const StageContent = opponent?.['num'] ?? MessageNumber++;

		//顔画像
		const FaceContent = targetCharacter ? LLSIdolManager.drawFace(targetCharacter.group_id, targetCharacter.id) : '';
...
function WriteSummary() {
	...
	const targetCharacters = LLSIdolManager.filterCharacterList("is:group_id:llsif","exhas:hidden");
	...
	const Output = summaryMap.keys().toArray().map(key => {
		return `<tr>
		<th>${LLSIdolManager.drawFace("llsif", key)}</th>
		<th>${LLSIdolManager.getCharacterData("llsif", key).name}</th>
