        ...
        if (isCallingTargets) {
            ...
            return `<tr><th${thProperties}><span class="shadowname">${callingCharacter.name}</span></th>${column.map(calledCharacterID => {
                ...
                const tdProperties = hasSpecificColor
                    ...
                    : ` class="calltable-BR" style="background-color: ${LLS.getColorFromColorCode(defaultBGColor, (isCallingSelf ? 0.6 : 3.5))}"`
                    ;
...
function initialize() {
    ...
    const MainCharacterList = LLSIdol.filterCharacterList('has:color_sifas');
    document.querySelector('style').textContent += MainCharacterList.map(character => {
		...
		.bg_calling_self_${character.id} {
			background-color: ${LLS.getColorFromColorCode(character['color_sifas'], 0.6, 0.1)};
		}
		.bg_calling_other_${character.id} {
			background-color: ${LLS.getColorFromColorCode(character['color_sifas'], 3.5)};
		}`
    }).join('');

    //表の描画
    document.getElementById('OutputAreaMuse').innerHTML =
        CreateTable(LLSIdol.filterCharacterList('is:group_id:muse').map(e => e.id), "#A0788C");

    document.getElementById('OutputAreaAqours').innerHTML =
        CreateTable(LLSIdol.filterCharacterList('is:group_id:aqours').map(e => e.id), "#788CA0");

    document.getElementById('OutputAreaNiji').innerHTML =
        CreateTable(LLSIdol.filterCharacterList('is:group_id:nijigasaki', 'isnot:id:Yu').map(e => e.id).concat("Player"), "#A0A08C");
